...
[@seo type = "brandContent"]
	<title>[#if seo.title??][@seo.title?interpret /][#else]${brand.name}[/#if][#if setting.isShowPowered] - ${setting.powered}[/#if]</title>
		...
		<div class="span18 last">
			<div class="path">
				<ul>
					...
					<li>
						<a href="${base}/brand/list/1.html">${message("shop.brand.title")}</a>
					</li>
				</ul>
			</div>
			<div class="top">
			...
			[#if brand.productCategories?has_content]
				<div class="product">
					<ul>
						<li class="title">
							<a href="${base}/product/list.html?brandId=${brand.id}">${message("shop.brand.product")}</a>
						</li>
