...
<title>${message("shop.cart.title")}[#if setting.isShowPowered] - ${setting.powered}[/#if]</title>
<meta name="author" content="progr1mmer" />
<link href="${base}/shop/css/common.css" rel="stylesheet" type="text/css" />
<link href="${base}/shop/css/cart.css" rel="stylesheet" type="text/css" />
		...
		<div class="span24">
			...
			[#if cart?? && cart.cartItems?has_content]
				...
				<div class="bottom">
					...
					<a href="${base}/order/info.html" id="submit" class="submit">${message("shop.cart.submit")}</a>
				</div>
			[#else]
				<p>
