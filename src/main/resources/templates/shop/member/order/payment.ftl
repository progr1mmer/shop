...
<title>${message("shop.order.payment")}[#if setting.isShowPowered] - ${setting.powered}[/#if]</title>
		...
		setInterval(function() {
			$.ajax({
				...
				success: function(data) {
					if (!data) {
						location.href = "view.html?sn=${order.sn}";
					}
				}
			});
		}, 10000);
...
<body>
	...
	<div class="container order">
		<div id="dialog" class="dialog">
			...
			<div>
				<a href="view.html?sn=${order.sn}">${message("shop.order.paid")}</a>
		...
		<div class="span24">
			...
			<div class="result">
				...
				<table>
					...
					<tr>
						...
						<td width="340">
							...
							<a href="view.html?sn=${order.sn}">[${message("shop.order.view")}]</a>
						</td>
				...
				[#if order.paymentStatus != "paid"]
					[#if order.paymentMethod.method == "online"]
						[#if paymentPlugins??]
							<form id="paymentForm" action="${base}/payment/submit" method="post" target="_blank">
