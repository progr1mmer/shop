...
<title>${message("admin.productNotify.list")}[#if setting.isShowPowered] - ${setting.powered}[/#if]</title>
...
<body>
	<div class="path">
		<a href="${base}/admin/common/index.html">${message("admin.path.index")}</a> &raquo; ${message("admin.productNotify.list")} <span>(${message("admin.page.total", page.total)})</span>
	</div>
				...
				<tr>
					...
					<td>
						[#if productNotify.member??]
							<a href="${base}/admin/member/view.html?id=${productNotify.member.id}">${productNotify.member.username}</a>
						[#else]
							-
						[/#if]
					</td>
