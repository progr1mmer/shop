...
<title>${message("admin.product.edit")}[#if setting.isShowPowered] - ${setting.powered}[/#if]</title>
...
<script type="text/javascript" src="${base}/admin/editor/kindeditor.js"></script>
	...
	$inputForm.validate({
		rules: {
			...
			sn: {
				pattern: /^[0-9a-zA-Z_-]+$/,
				remote: {
					url: "check_sn?previousSn=${product.sn}",
					cache: false
				}
			},
			price: {
...
<body>
	<div class="path">
		<a href="${base}/admin/common/index.html">${message("admin.path.index")}</a> &raquo; ${message("admin.product.edit")}
	</div>
	<form id="inputForm" action="update" method="post" enctype="multipart/form-data">
		...
		<table class="input tabContent">
            ...
            <tr>
                ...
                <td>
					<span class="fieldSet">
						<input type="text" name="imageI" class="text" value="${product.imageI}" maxlength="200" title="${message("admin.product.imageTitle")}" />
						<input type="button" id="browserButtonI" class="button" value="${message("admin.browser.select")}" />
						[#if product.imageI??]
							<a href="${product.imageI}" target="_blank">${message("admin.common.view")}</a>
                        [/#if]
					</span>
                </td>
            </tr>
		...
		<table class="input tabContent">
            ...
            <tr>
                ...
                <td>
                    <textarea id="editorB" name="introductionB" class="editor" style="width: 100%;">${product.introductionB?html}</textarea>
                </td>
            </tr>
            [#--<tr>
                <th>
		...
		<table id="productImageTable" class="input tabContent">
			...
			[#list product.productImages as productImage]
				<tr>
                    ...
                    <td>
                        <input type="checkbox" name="productImages[${productImage_index}].isB" value="true"[#if productImage.isB] checked="checked"[/#if] />
                    </td>
					<td>
						<a href="javascript:" class="deleteProductImage">[${message("admin.common.delete")}]</a>
					</td>
					...
					<table id="specificationProductTable" class="input">
						...
						[#list product.siblings as specificationProduct]
							<tr>
								...
								<td>
									<a href="javascript:;" class="deleteSpecificationProduct">[${message("admin.common.delete")}]</a>
									<a href="edit.html?id=${specificationProduct.id}">[${message("admin.common.edit")}]</a>
								</td>
							</tr>
						[/#list]
					</table>
		...
		<table class="input">
			<tr>
				...
				<td>
					...
					<input type="button" class="button" value="${message("admin.common.back")}" onclick="location.href='list.html'" />
				</td>
			</tr>
		</table>
	</form>
