...
<title>${message("admin.order.list")}[#if setting.isShowPowered] - ${setting.powered}[/#if]</title>
...
<script type="text/javascript" src="${base}/admin/js/list.js"></script>
...
<body>
	<div class="path">
		<a href="${base}/admin/common/index.html">${message("admin.path.index")}</a> &raquo; ${message("admin.order.list")} <span>(${message("admin.page.total", page.total)})</span>
	</div>
					...
					[@shiro.hasPermission name = "admin:print"]
						<td>
							<select name="print">
								...
								<option value="../print/order.html?id=${order.id}">${message("admin.order.orderPrint")}</option>
								<option value="../print/product.html?id=${order.id}">${message("admin.order.productPrint")}</option>
								<option value="../print/shipping.html?id=${order.id}">${message("admin.order.shippingPrint")}</option>
								<option value="../print/delivery.html?orderId=${order.id}">${message("admin.order.deliveryPrint")}</option>
							</select>
						</td>
					[/@shiro.hasPermission]
					<td>
						<a href="view.html?id=${order.id}">[${message("admin.common.view")}]</a>
						[#if !order.expired && order.orderStatus == "unconfirmed"]
							<a href="edit.html?id=${order.id}">[${message("admin.common.edit")}]</a>
						[#else]
