...
<title>${message("admin.deposit.list")}[#if setting.isShowPowered] - ${setting.powered}[/#if]</title>
...
<body>
	<div class="path">
		<a href="${base}/admin/common/index.html">${message("admin.path.index")}</a> &raquo; ${message("admin.deposit.list")} <span>(${message("admin.page.total", page.total)})</span>
	</div>
	<form id="listForm" action="list.html" method="get">
		...
		<div class="bar">
			<div class="buttonWrap">
				[#if member??]
					<a href="../member/view.html?id=${member.id}" class="button">${message("admin.common.back")}</a>
				[/#if]
		...
		<table id="listTable" class="list">
			...
			[#list page.content as deposit]
				<tr>
					...
					<td>
						[#if deposit.member??]
							<a href="../member/view.html?id=${deposit.member.id}">${deposit.member.username}</a>
						[#else]
							-
						[/#if]
					</td>
					<td>
						[#if deposit.order??]
							<a href="../order/view.html?id=${deposit.order.id}">${deposit.order.sn}</a>
						[#else]
							-
						[/#if]
					</td>
					<td>
						[#if deposit.payment??]
							<a href="../payment/view.html?id=${deposit.payment.id}">${deposit.payment.sn}</a>
						[#else]
							-
						[/#if]
					</td>
