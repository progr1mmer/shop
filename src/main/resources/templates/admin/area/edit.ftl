...
<title>${message("admin.area.edit")}[#if setting.isShowPowered] - ${setting.powered}[/#if]</title>
...
<body>
	<div class="path">
		<a href="${base}/admin/common/index.html">${message("admin.path.index")}</a> &raquo; ${message("admin.area.edit")}
	</div>
		...
		<table class="input">
			...
			<tr>
				...
				<td>
					...
					<input type="button" class="button" value="${message("admin.common.back")}" onclick="location.href='list.html[#if area.parent??]?parentId=${area.parent.id}[/#if]'" />
				</td>
			</tr>
		</table>
	</form>
