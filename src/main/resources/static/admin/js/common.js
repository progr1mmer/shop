...
var messages = {
	...
	"admin.dialog.copyConfirm": "您确定要复制吗？",
	...
	$.fn.extend({
		browser: function(options) {
			var settings = {
				...
				isUpload: true,
				browserUrl: shop.base + "/admin/file/browser",
				uploadUrl: shop.base + "/admin/file/upload",
				callback: null
			};
			$.extend(settings, options);
					...
					function createBrowserList(path, data) {
						...
						$.each(data, function(i, fileInfo) {
							...
							if (fileInfo.isDirectory) {
								iconUrl = shop.base + "/admin/images/folder_icon.gif";
								title = fileInfo.name;
								...
								iconUrl = shop.base + "/admin/images/file_icon.gif";
								title = fileInfo.name + " (" + Math.ceil(fileInfo.size / 1024) + "KB, " + new Date(fileInfo.lastModified).toLocaleString() + ")";
							}
