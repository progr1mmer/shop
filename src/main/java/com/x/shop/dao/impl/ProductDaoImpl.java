...
public class ProductDaoImpl extends BaseDaoImpl<Product, Long> implements ProductDao {
    ...
    public List<Object[]> findSalesList(Date beginDate, Date endDate, Integer count) {
        ...
        Join<Product, com.x.shop.entity.Order> order = orderItems.join("order");
        ...
        restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.equal(order.get("orderStatus"), com.x.shop.entity.Order.OrderStatus.completed), criteriaBuilder.equal(order.get("paymentStatus"), com.x.shop.entity.Order.PaymentStatus.paid));
        criteriaQuery.where(restrictions);
    ...
    public boolean isPurchased(Product product) {
        ...
        //TODO
        //Long count = entityManager.createQuery(jqpl, Long.class).setFlushMode(FlushModeType.COMMIT).setParameter("product", product).setParameter("member", member).setParameter("orderStatus", com.x.shop.entity.Order.OrderStatus.completed).getSingleResult();
        Long count = entityManager.createQuery(jqpl, Long.class).setFlushMode(FlushModeType.COMMIT).setParameter("product", product).setParameter("orderStatus", com.x.shop.entity.Order.OrderStatus.completed).getSingleResult();
        return count > 0;
    }
